...
evmc_status_code check_requirements(
    ...
    //HACK BEGIN: We should do something like this here
    ...
    auto opcode_name = get_opcode_name(opcode_names_, Op);
    if (!opcode_name) {
        opcode_name = get_opcode_name(latest_opcode_names_, Op);
    }
...
void CallTracer::on_instruction_start(uint32_t pc, const intx::uint256* stack_top, int stack_height, int64_t gas,
    ...
    auto opcode_name = get_opcode_name(opcode_names_, op_code);
    if (!opcode_name) {
        opcode_name = get_opcode_name(latest_opcode_names_, op_code);
    }
