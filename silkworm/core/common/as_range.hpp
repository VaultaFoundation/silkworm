...
OutputIterator move(InputRange& range, OutputIterator result) {
    return std::move(std::begin(range), std::end(range), result);
}

template <typename Cont, typename Compare>
decltype(auto) max_element(Cont& c, Compare&& comp) {
    return std::max_element(std::begin(c), std::end(c), std::forward<Compare>(comp));
}
