...
Task<intx::uint256> EstimateGasOracle::estimate_gas(const Call& call, const silkworm::Block& block, std::optional<TxnId> txn_id, std::optional<BlockNum> block_num_for_gas_limit) {
    ...
    const auto [evm_version, gas_params, gas_prices_orig] = co_await load_gas_parameters(transaction_, config_, block);
    ...
    auto gas_prices = gas_prices_orig;
    if( evm_version >= eosevm::EVM_VERSION_3 && call.gas.has_value() && call.gas.value() == 0 ) {
