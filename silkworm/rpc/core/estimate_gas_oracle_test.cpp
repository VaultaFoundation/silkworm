...
TEST_CASE("estimate gas") {
    ...
    SECTION("Conservative gas: check failure when gas=0 and inclusio_price>0") {
        MockEstimateGasOracle oracle{block_header_provider, account_reader, kEOSEVMMainnetConfig, workers, *tx, storage, accounts_overrides};

        silkworm::Block block2;
        block2.set_gas_prices({.overhead_price=1, .storage_price=1});
        block2.header.number = 1;
        block2.header.nonce = eosevm::version_to_nonce(eosevm::EVM_VERSION_3);
        try {
            call.gas = 0;
            call.max_priority_fee_per_gas = 1;
            call.max_fee_per_gas = 2;
            EXPECT_CALL(oracle, try_execution(_, _)).Times(testing::AnyNumber());
            auto result = boost::asio::co_spawn(pool, oracle.estimate_gas(call, block2, {}), boost::asio::use_future);
            result.get();
            CHECK(false);
